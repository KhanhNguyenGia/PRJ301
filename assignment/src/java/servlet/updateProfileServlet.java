...
public class updateProfileServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            String newFullname = (String) request.getParameter("fullname");
            ...
            try {
                ...
                if (acc == null) {
                    ...
                    if (newFullname == null || newFullname.isEmpty()) {
                        newFullname = acc.getFullname();
                    }
                    if (newPhone == null || newPhone.isEmpty()) {
                        newPhone = acc.getPhone();
                    }
                    boolean res = AccountDAO.updateAccount(email, password, newFullname, newPhone);
                    if (res) {
                        session.setAttribute("name", newFullname);
                        session.setAttribute("error", "Update account successfully");
                    } else {
                        session.setAttribute("error", "Fail to update account");
                    }
                    request.getRequestDispatcher("changeProfile.jsp").forward(request, response);
                }
            } catch (Exception e) {
